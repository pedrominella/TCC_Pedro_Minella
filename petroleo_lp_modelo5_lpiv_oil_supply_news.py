# -*- coding: utf-8 -*-
"""
petroleo_lp_modelo5_lpiv_oil_supply_news.py

MODELO 5 - LOCAL PROJECTIONS COM CHOQUE EXTERNO DE OFERTA DE PETRÓLEO
...
O Modelo 5 possui duas partes:

1. Modelo 5A - Local Projections diretas:
...
- Se o primeiro estágio for fraco, use o Modelo 5A como evidência externa e trate o LP-IV com cautela.
- O Modelo 5 deve ser tratado como robustez de identificação, não como substituto do Modelo 3 e do Modelo 4.
...
OUTPUT_DIR = Path("output_petroleo_lp_modelo5_lpiv_oil_supply_news")
...
def juntar_bases():
    ...
    base.to_excel(OUTPUT_DIR / "base_modelo5_com_choque_oferta.xlsx", index=False)
...
# =============================================================================
# 4. MODELO 5A - LP DIRETA COM CHOQUE EXTERNO
# =============================================================================
...
def local_projection_direta(
    ...
    ax.set_title(f"Modelo 5A - LP direta - {nome_modelo}")
...
def lp_iv_por_horizonte(
    ...
    ax.axhline(0, linewidth=1)
    ax.axvline(H_PRINCIPAL, linestyle="--", linewidth=1, alpha=0.6)
    ax.set_title(f"Modelo 5B - LP-IV - {nome_modelo}")
    ax.set_xlabel("Horizonte h, em meses")
    ax.set_ylabel("Resposta acumulada")
    ax.grid(True, alpha=0.3)
    ax.legend()
...
def estimar_modelo5A(df, vars_t, dummy_cols):
    print("\n" + "=" * 100)
    print("4) MODELO 5A - LP DIRETA COM OIL SUPPLY NEWS SHOCK")
    print("=" * 100)
    ...
    for alvo in alvos:
        ...
        print(f"Estimando Modelo 5A: {nome}")
...
def estimar_modelo5B(df, vars_t, dummy_cols):
    print("\n" + "=" * 100)
    print("5) MODELO 5B - LP-IV COM OIL SUPPLY NEWS SHOCK COMO INSTRUMENTO")
    print("=" * 100)

    if "petroleo_brl" not in vars_t:
        print("Não há dln_petroleo_brl na base. Modelo 5B não será estimado.")
        return
    ...
    for alvo in alvos:
        ...
        print(f"Estimando Modelo 5B: {nome}")
...
def criar_resumos_modelo5():
    print("\n" + "=" * 100)
    print("6) CRIANDO RESUMOS DO MODELO 5")
    print("=" * 100)
    ...
    if resumo:
        ...
        resumo_df.to_csv(OUTPUT_DIR / "resumo_modelo5_resultados_horizontes.csv", index=False, encoding="utf-8-sig")
        resumo_df.to_excel(OUTPUT_DIR / "resumo_modelo5_resultados_horizontes.xlsx", index=False)

    if resumo_fs:
        fs_df = pd.DataFrame(resumo_fs)
        fs_df.to_csv(OUTPUT_DIR / "resumo_modelo5_primeiro_estagio.csv", index=False, encoding="utf-8-sig")
        fs_df.to_excel(OUTPUT_DIR / "resumo_modelo5_primeiro_estagio.xlsx", index=False)

    texto = []
    texto.append("RELATÓRIO DO MODELO 5\n")
    texto.append("=====================\n\n")
    texto.append("O Modelo 5 usa a série Oil supply news shock como fonte externa de identificação.\n\n")
    texto.append("Modelo 5A:\n")
    ...
    texto.append("Como avaliar o Modelo 5B:\n")
    texto.append("- Primeiro olhe o arquivo resumo_modelo5_primeiro_estagio.xlsx.\n")
    texto.append("- Se o F-stat do primeiro estágio for maior que 10, o instrumento é mais defensável.\n")
    texto.append("- Se o F-stat for menor que 10, trate o LP-IV com cautela e use o Modelo 5A como evidência externa.\n\n")
    ...
    (OUTPUT_DIR / "RELATORIO_MODELO5.txt").write_text("".join(texto), encoding="utf-8")
...
def main():
    ...
    estimar_modelo5A(df, vars_t, dummy_cols)
    estimar_modelo5B(df, vars_t, dummy_cols)
    criar_resumos_modelo5()

    print("\n" + "=" * 100)
    print("MODELO 5 FINALIZADO")
    print("=" * 100)
    print(f"Arquivos salvos em: {OUTPUT_DIR.resolve()}")

    print("\nO que olhar primeiro:")
    print("1. resumo_modelo5_primeiro_estagio.xlsx")
    print("2. resumo_modelo5_resultados_horizontes.xlsx")
