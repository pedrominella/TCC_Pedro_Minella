...
def clean_exog(df):
    ...
    # remove colunas constantes/all-zero na amostra usada
    nun = x.nunique(dropna=False)
    x = x.loc[:, nun > 1]
    # remove colunas duplicadas exatas
...
df = df[(df.index >= pd.Timestamp(DATA_INICIO)) & (df.index <= pd.Timestamp(DATA_FINAL))].copy()
...
for comb_name, comb_col in comb_map.items():
    for resp_name, resp_col in resp_map.items():
        ...
        # limpa exógenas novamente na amostra efetiva do modelo
        if X.shape[1] > 0:
            X = clean_exog(X)
        ...
        with open(OUTDIR / "modelos" / f"resumo_{nome}.txt", "w", encoding="utf-8") as f:
            try:
                ...
                f.write(f"Resumo completo indisponível: {e}\n\n")
                f.write(f"Modelo: {nome}\n")
                f.write(f"N obs: {res.nobs}\n")
                f.write(f"Lag: {lag}\n")
                f.write(f"AIC: {res.aic}\n")
                f.write(f"BIC: {res.bic}\n")
                f.write(f"HQIC: {res.hqic}\n")
                f.write(f"Raízes: {res.roots}\n")
