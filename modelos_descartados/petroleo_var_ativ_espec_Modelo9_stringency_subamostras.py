...
MAXLAGS = 12
HORIZONTE_IRF = 24
OUTDIR_BASE = Path("output_petroleo_var_ativ_espec_Modelo9_stringency_subamostras")
...
def carregar_preparar_base():
    ...
    # Modelo 9: Selic retirada das equações endógenas do VAR.
    base_vars = [
        "Preco_Barril",
        "Cambio",
        "GasolinaA_nivel",
...
def rodar_subamostra(work_full, base_vars, map_cols, nome_sub, data_ini, data_fim):
    ...
    for comb_name, comb_col in comb_map.items():
        for resp_name, resp_col in resp_map.items():
            ...
            fig, axes = plt.subplots(len(resid.columns), 1, figsize=(10, 12), sharex=True)
